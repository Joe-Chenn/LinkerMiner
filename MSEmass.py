...
class CEmass:
	...
	def __soldierGenerateVerginList(self, inputElementName):
		...
		SuperAtomList = []
		numberAtomType = len(self.dp.myINI.DIC_ELEMENT_ALL_MASS[inputElementName])  # 如果元素不在element.ini中，就会报错

		for i in range(numberAtomType):  # 对于同一个元素中的多种同位素处理, 变成[丰度, 质量]的list

			tmpMass = self.dp.myINI.DIC_ELEMENT_ALL_MASS[inputElementName][i]
			tmpPro = self.dp.myINI.DIC_ELEMENT_MASS_PERCENTAGE[inputElementName][i]
	...
	def getCalculatedMonoMZ(self, inputDictCom, inputCharge):
		...
		for name in inputDictCom:
			...
			list_abdc = self.dp.myINI.DIC_ELEMENT_MASS_PERCENTAGE[name]
			list_mass = self.dp.myINI.DIC_ELEMENT_ALL_MASS[name]
